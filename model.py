...
def model7(X,y,seed,is_training):
    ...
    pool3 = tf.layers.max_pooling2d(inputs=relu3, pool_size=[2, 2], strides=2)
    ...
    pool4 = tf.layers.max_pooling2d(inputs=relu4, pool_size=[2, 2], strides=2)

    # Dense Layer
    pool_flat = tf.reshape(pool4, [-1, 3 * 3 * 256])    # put 12 X 12 X {the value of number of filters of last layer}
    dense = tf.layers.dense(inputs=pool_flat, units=1024,use_bias=True,kernel_initializer=initializer)
    batch_norm5 = tf.layers.batch_normalization(dense,training=is_training)
    relu5 = tf.nn.relu(batch_norm5)
    dropout = tf.layers.dropout(inputs=relu5, rate=0.6, training=is_training)
